...
index_list <- c(); region_list <- c(); cluster_list <- c(); cor04_list <- c()
for (r in c("Africa","Americas","Asia","Europe","Oceania", "Northern America ")){
  ...
  for (cluster in as.numeric(names(table(y$region)))){
    ...
    pca_rand95_long<-
      gather(pca_rand95, key = Variable, value = Value, -PC)
    ...
    # Save the all PCs of each cluster
    write.csv(pca_prcomp$x, file=paste0("pc_data/", r, cluster,".csv"), quote = F, row.names = F)
    ...
    c <- 
    for (pc in 1:length(oscillations$year)){
...
summary_ccf <- data.frame(index=index_list, region=region_list, cluster=cluster_list, cor=cor04_list)
write.csv(summary_ccf, file = "summary_ccf.csv", quote = F)
