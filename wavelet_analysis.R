...
summary_pca <- read.csv("summary_ccf.csv")
...
#wt.image(wavelet_results$PC1_oni$cross_wavelet,  main = paste("Cross-Wavelet Power Spectrum of ", r, "for ", pc, "and", index))

# Plot the Results
#plot_wavelet <- function(result, pc, index) {
  #par(mfrow = c(3, 1))
  #wt.image(result$wavelet_pc, main = paste("Wavelet Power Spectrum of", pc))
  #wt.image(result$wavelet_index, main = paste("Wavelet Power Spectrum of", index))
#  wt.image(result$cross_wavelet, main = paste("Cross-Wavelet Power Spectrum of", pc, "and", index))
#}

#dir.create("wavelet")
#pdf("wavelet/wavelet_plots.pdf")
#for (result in names(wavelet_results)) {
#  pc_index <- strsplit(result, "_")[[1]]
#  plot_wavelet(wavelet_results[[result]], pc_index[1], pc_index[2])
#}
#dev.off()
